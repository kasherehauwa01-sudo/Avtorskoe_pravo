...
5. Для каждой строки из Excel:
   ...
   - если код найден — готовит обновление `Поставщик` и `Менеджер`;
   - если код не найден — готовит добавление новой строки и заливку диапазона `A:G`.
6. После разбора всего Excel приложение выполняет пакетную запись в Google Таблицу, чтобы уменьшить количество запросов к API и избежать ошибок по квоте `Write requests`.
7. В режиме `Без записи` приложение формирует только лог без отправки изменений в Google Sheets.
8. Показывает лог выполнения в отдельном компактном окне с собственной прокруткой.
...
- Ошибка квоты `Write requests` → приложение уже использует пакетную запись; если лимит всё равно достигнут, повторите запуск позже или уменьшите частоту массовых загрузок.
...
- пакетная запись изменений в Google Sheets для уменьшения числа API-запросов.
